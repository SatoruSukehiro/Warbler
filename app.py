...
def users_show(user_id):
    ...
    return render_template('users/show.html', user=user, messages=messages, total_likes=total_likes)
...
def profile():
    ...
    if form.validate_on_submit():
        ...
        checked_user = User.authenticate(username=form.username.data,password=form.password.data)
        if checked_user:
            db.session.commit()
            return redirect(f'/users/{checked_user.id}')
        else:
            flash('There Seems to be an Error',"danger")
            redirect('/')
...
def remove_like(message_id):
    selected_like = Likes.query.filter_by(message_id=message_id).first()
    message = Message.query.get(message_id)
    db.session.delete(selected_like)
    db.session.commit()
    flash(f'Unliked Warble created by:{message.user.username}','success')
...
def homepage():
    ...
    if g.user:
        ...
        print([message.user_id for message in messages])
